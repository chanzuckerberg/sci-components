...
<p>
  Raw PDB text is the only required prop. Supplying per-residue
...
<p>
  Selection is controlled through one prop. Every way of making one - clicking a
  residue, dragging across the sequence, clicking a chain's caption - arrives
  through <code>onSelectionChange</code>; echoing it back through
  <code>selection</code> frames it, and setting <code>null</code> zooms back
  out. Clicking empty space calls <code>onSelectionChange</code> with
  <code>null</code>.
</p>
<p>
...
<p>
  ...
  PDB string and the viewer finds the chains itself, reporting them through
...
<p>
  ...
  panel, lights that chain's cartoon up in the 3D view. The highlight is pink
  rather than the theme's usual hover tint, so it cannot be mistaken for the
  chain palette, the pLDDT bands or an overlay underneath it. Keyboard focus
  does the same, and nothing is reported to the consumer - it answers "which one
  is this?" without changing what is selected.
...
<p>
  Whatever a PDB entry carries besides its protein - ligands, ions, glycans,
  lipids - is drawn as ball-and-stick alongside the cartoon, with no prop to
  turn on. A cartoon traces a polymer backbone and a heme has none, so the two
  halves of a chain need two representations between them.
</p>
<p>
...
<p>
  ...
  panel, the fade that masks residues scrolling under the header, and the "no
  structure available" state, so the panel stays one color throughout.
</p>
...
<p>
  ...
  away: the viewer removes Mol*'s click-to-zoom camera so that
  <code>selection</code> alone drives the camera, and that stays removed.
</p>
<p>
...
<table class="sds-doc-table">
  ...
  <tr>
    ...
    <td><p>- (required)</p></td>
    <td><p>The structure to render, as raw PDB text.</p></td>
  </tr>
  <tr>
    <td>
      <p><code>plddt</code></p>
  ...
  <tr>
    ...
    <td>
      <p>
        What is selected. Controlled: the camera frames whatever it covers, and
        clearing it zooms back out. See the table below for its shape.
      </p>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>
      <p>
        Stop a chain from lighting up in the 3D view while its name is pointed
        at, in the legend or above its grid in the sequence panel. Turn it off
        where the movement is more distracting than the answer is useful.
      </p>
    </td>
  </tr>
  <tr>
