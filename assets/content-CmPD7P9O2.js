...
<p>
  ...
  per chain, splits the sequence panel into one grid per chain, and - when chain
  coloring is what paints the structure - paints each chain its own color and
  shows a chain legend with a swatch and a visibility toggle for each.
</p>
<p>
...
<h3>Representation, coloring and highlights</h3>
<p>
  <code>representation</code> draws the polymer as a cartoon, the default, or as
  a molecular surface. The surface is one surface over every visible chain
  rather than one per chain, so where two chains meet the interface is buried -
  and hiding a partner rebuilds it over what is left, exposing the face the
  partner was bound to. Ligands stay ball-and-stick either way. A surface too
  large to compute keeps the cartoon in its place and is reported to
  <code>onError</code> as a <code>"representation"</code> failure.
</p>
<p>
  <code>colorBy</code> names what paints the structure. Left undefined it
  follows what is supplied - an overlay, then pLDDT, then chain colors - and
  naming one is how a toggle keeps both <code>plddt</code> and
  <code>chainColors</code> in place while it switches between them. The legend
  follows whichever is painting. A <code>null</code> in <code>plddt</code>, or a
  residue past its end, reads as unscored and is painted neutral rather than at
  the bottom of the scale, and the structure text is parsed exactly as given:
  scores are read by residue, not written into the file's B-factor column.
</p>
<p>
  <code>highlights</code> picks residues out in colors of their own, by the
  chain, number and insertion code the file gives them. They paint over whatever
  colors the rest, and are drawn in ball-and-stick over the cartoon; a surface
  carries them in its own colors. Highlights that name no color take the
  viewer's palette in turn, which is exported as
  <code>HIGHLIGHT_COLOR_PALETTE</code> for a legend of your own. They are
  separate from the selection: highlighting a residue neither selects it nor
  moves the camera. <code>selection.addresses</code> selects by the same kind of
  address.
</p>
<p>
  Every one of these changes is made in place, so the camera stays wherever it
  was left.
</p>
<div
  class="sds-doc-example"
  data-example="data-viz/ProteinStructureViewer/SceneProteinStructureViewer"
></div>
<h3>Camera</h3>
<p>
  <code>orientation</code> turns the camera to look at the highlighted residues:
  <code>"facing"</code> looks at them from outside the structure, down the line
  from its center through theirs; <code>"opposite"</code> looks from the far
  side; <code>"side"</code> looks across that line, which puts an interface in
  profile; and <code>"overview"</code> looks down the default axis at the whole
  structure. It is applied when a structure loads and whenever it changes - not
  when the highlights do. <code>projection</code> sets the camera's projection.
</p>
<p>
  <code>onCameraChange</code> reports the camera each time it comes to rest,
  whether after a drag, a zoom, a selection framing it or a structure fitting
  it, as a <code>CameraState</code>. Handing that back as
  <code>initialCamera</code> starts the next structure loaded exactly there, in
  place of fitting it. <code>onStructureLoad</code> is told of every structure
  that loads, reloads included, which is what a "loading" and "ready" state
  listens for.
</p>
...
<table class="sds-doc-table">
  ...
  <tr>
    ...
    <td>
      <p><code>(plugin: PluginUIContext) =&gt; Promise&lt;Blob&gt;</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Renders the image in place of the viewer's own capture, and resolves
        with a PNG. <code>resolution</code>, <code>backgroundColor</code> and
        <code>showAxes</code> describe the viewer's capture, so they are not
        applied to this one.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        <code
          >(image: Blob, filename: string) =&gt; void |
          Promise&lt;void&gt;</code
        >
      </p>
    </td>
    ...
    <td>
      <p>
        Receives the PNG in place of a browser download, with the name it would
        have been saved under, <code>.png</code> included.
      </p>
    </td>
  </tr>
</table>
<p>
  The last two take the capture over, and either can be given without the other.
  <code>deliver</code> is for a page that cannot start a download itself: a
  sandboxed iframe blocks the one a click would start, so the image is handed to
  a bridge to its host instead. <code>render</code> is for an image the presets
  do not describe - bounded to a size, or keeping the viewport's aspect. A
  failure in either is reported to <code>onError</code>.
</p>
<div class="sds-doc-code-snippet">
  <figure>
    <figcaption>React TypeScript</figcaption>
...
<h2>Building on the viewer</h2>
<p>
  <code>onReady</code> hands over the Mol* plugin once a structure is loaded,
  and again for each structure that replaces it, along with the structure the
  viewer parsed. That is the way into anything the props do not cover: drawing a
  representation of your own, registering a color theme, or driving the camera.
  It is awaited before the viewer starts answering clicks and applying its
  coloring, so whatever it builds is in place before either reaches it.
</p>
<p>
  To draw the whole scene yourself, set <code>sceneMode="external"</code>. The
  viewer then parses the structure and stops there: nothing is drawn, colored,
  hidden or framed, and the canvas is yours. Everything that is not drawing
  keeps working - the sequence panel, selection, hover, the legend and the
  camera controls. The chain legend's toggles report through
  <code>onChainVisibilityChange</code> without hiding anything, since what
  hiding a chain means is now yours to decide, and the legend describes the
  props rather than what you drew, so pass it the colors you draw with. Mol*
  frames the camera on the first representation drawn into the empty scene.
</p>
<div
  class="sds-doc-example"
  data-example="data-viz/ProteinStructureViewer/ExternalSceneProteinStructureViewer"
></div>
<p>
  Whatever is built on a structure goes with it when the next one is loaded, so
  build it again in the next <code>onReady</code> rather than holding on to it.
  A component keyed <code>polymer-&lt;chainId&gt;</code> or
  <code>ligand-&lt;chainId&gt;</code> is dimmed along with its chain while
  another chain's name is pointed at.
</p>
<p>
  <code>onDispose</code> is called before the viewer disposes of a plugin it
  handed over - including one whose <code>onReady</code> has not finished - so
  whatever was built on it can be let go. <code>onError</code> is told when
  creating the plugin, loading a structure or capturing an image fails, in place
  of the message the viewer would otherwise log; a rejected
  <code>onReady</code> counts as a failed load.
</p>
<h2>Rendering without React</h2>
<p>
  The scene the viewer draws is available on its own, for a page that does not
  show the viewer: a report with a picture of the structure, a static export, or
  a Mol* viewer of your own. It takes the options the props take, with the same
  meaning, and draws them by the same code, so what it draws matches the viewer
  given the same props. Import it from its own entry point, which leaves out
  React and the component library:
</p>
<div class="sds-doc-code-snippet">
  <figure>
    <figcaption>TypeScript</figcaption>
    <pre><code class="sds-doc-codeblock-content language-ts">import {
  applyStructureScene,
  renderStructureImage,
} from "@czi-sds/data-viz/ProteinStructureScene";</code></pre>
  </figure>
</div>
<p>
  <code>renderStructureImage</code> renders a PNG of exactly
  <code>width</code> by <code>height</code> pixels. It creates a plugin
  offscreen for the one image and disposes of it once the image is taken,
  whether or not that succeeded, so nothing is left on the page. The camera is
  placed as the viewer would place it when the structure loads, without the
  animation. Leave out <code>backgroundColor</code> for a transparent
  background.
</p>
<div class="sds-doc-code-snippet">
  <figure>
    <figcaption>TypeScript</figcaption>
    <pre><code class="sds-doc-codeblock-content language-ts">const image: Blob = await renderStructureImage({
  structure: pdb,
  plddt: scores,
...
<p>
  It needs a DOM and WebGL: a browser, or headless Chromium on a server. Where
  there is no WebGL it rejects rather than returning a blank image.
</p>
<p>
  <code>applyStructureScene</code> draws the scene on a Mol* plugin you created
  and resolves with a handle once the structure is drawn and the camera is on
  it. <code>update</code> applies changed options in place and leaves the camera
  where it is, except for a new <code>structure</code>, which is loaded and
  framed like the first. <code>getCamera</code> reads the camera in the form
  <code>initialCamera</code> takes, and <code>dispose</code> stops the scene
  touching the plugin, which stays yours.
</p>
<div class="sds-doc-code-snippet">
  <figure>
    <figcaption>TypeScript</figcaption>
    <pre><code class="sds-doc-codeblock-content language-ts">const scene = await applyStructureScene(plugin, { structure: pdb });

await scene.update({ representation: "surface", hiddenChains: ["B"] });
const camera = scene.getCamera();

scene.dispose();</code></pre>
  </figure>
</div>
<p>
  The scene clears the plugin's state to load a structure, so give it a plugin
  of its own rather than one showing something else. Its color themes are
  registered once per plugin, so applying a scene again on the same plugin is
  fine. A structure that fails to load rejects the promise; a representation
  that cannot be drawn, such as a surface too large to compute, is reported to
  <code>onError</code> with the phase <code>"representation"</code> and the
  cartoon is drawn in its place.
</p>
<h2>Props</h2>
<p>
...
<table class="sds-doc-table">
  ...
  <tr>
    ...
    <td>
      <p><code>(number | null)[] | null</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        ...
        <code>residueOverlay</code> takes over. A <code>null</code>, and every
        residue past the end of the array, reads as unscored and is painted
        neutral.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>colorBy</code></p>
    </td>
    <td>
      <p><code>"chain" | "plddt" | "overlay"</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        What paints the structure. Left undefined it follows what is supplied:
        <code>residueOverlay</code>, then <code>plddt</code>, then chain colors.
        A theme with nothing to paint with leaves every residue neutral.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>representation</code></p>
    </td>
    <td>
      <p><code>"cartoon" | "surface"</code></p>
    </td>
    <td>
      <p><code>"cartoon"</code></p>
    </td>
    <td>
      <p>
        How the polymer is drawn. <code>"surface"</code> is one molecular
        surface over the visible chains. Switching leaves the camera where it
        is.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p><code>ResidueHighlight[]</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Residues picked out in colors of their own, by chain, number and
        insertion code, painted over whatever colors the rest and drawn in
        ball-and-stick over the cartoon. See the table below for its shape.
      </p>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>
      <p>
        ...
        chain coloring is what is on screen: when <code>colorBy</code> says so
        or, left undefined, when neither <code>plddt</code> nor
        <code>residueOverlay</code> is set.
      </p>
    </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>
      <p><code>"perspective" | "orthographic"</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>The camera's projection. Left undefined, Mol*'s own default stands.</p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>initialCamera</code></p>
    </td>
    <td>
      <p><code>CameraState | null</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Where the camera starts on each structure loaded, in place of fitting
        it. Takes what <code>onCameraChange</code> reports.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p><code>"overview" | "facing" | "opposite" | "side"</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Turns the camera to look at the highlighted residues. Applied when a
        structure loads, unless <code>initialCamera</code> is set, and whenever
        it changes.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        <code>(camera: CameraState) =&gt; void</code>. Called with the camera
        each time it comes to rest after moving.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>onStructureLoad</code></p>
    </td>
    <td>
      <p><code>function</code></p>
    </td>
    ...
    <td>
      <p>
        <code>(info: StructureLoadInfo) =&gt; void</code>. Called for every
        structure that loads, reloads included, with its atom and residue counts
        and its chains.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>sceneMode</code></p>
    </td>
    <td>
      <p><code>"managed" | "external"</code></p>
    </td>
    <td>
    ...
    <td>
      <p>
        Who draws the structure. <code>"external"</code> parses it and leaves
        the canvas empty for a scene drawn in <code>onReady</code>; see Building
        on the viewer. Read once, when the plugin is created.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>onReady</code></p>
    </td>
    <td>
      <p><code>function</code></p>
    </td>
    ...
    <td>
      <p>
        <code
          >(plugin: PluginUIContext, loaded: LoadedStructureInfo) =&gt; void |
          Promise&lt;void&gt;</code
        >. Called with the Mol* plugin once a structure is loaded, and again for
        each structure that replaces it. Awaited before the viewer answers
        clicks or applies its coloring. Not called for a structure that failed
        to load.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        <code
          >(error: unknown, phase: "init" | "load" | "capture" |
          "representation") =&gt; void</code
        >. Called when creating the plugin, loading a structure, capturing an
        image or drawing a representation fails, in place of the viewer's
        console message. A rejected <code>onReady</code> is a
        <code>"load"</code> failure.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>onDispose</code></p>
    </td>
    <td>
      <p><code>function</code></p>
    </td>
    ...
    <td>
      <p>
        <code>() =&gt; void</code>. Called before the viewer disposes of a
        plugin it handed to <code>onReady</code>, including one whose
        <code>onReady</code> has not finished.
      </p>
    </td>
  </tr>
</table>
...
<table class="sds-doc-table">
  ...
  <tr>
    ...
    <td>
      <p><code>ResidueAddress[]</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Residues by the chain, number and insertion code the file gives them,
        for a caller that knows a residue by its address rather than its
        position. They join <code>residues</code>; an address the structure does
        not have selects nothing. The viewer reports its own selections as
        <code>residues</code>.
      </p>
    </td>
  </tr>
...
<h3>ResidueHighlight</h3>
<p>
  A residue by its address - a <code>ResidueAddress</code>, which is the same
  three fields without <code>color</code> - and the color it is picked out in.
  <code>ResidueRef</code> reports the same three, so a residue the viewer
  reported can be handed straight back.
</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
  ...
  <tr>
    ...
    <td><p>- (required)</p></td>
    <td><p>Chain as named in the file (<code>auth_asym_id</code>).</p></td>
  </tr>
  <tr>
    <td>
      <p><code>seqId</code></p>
    ...
    <td><p>- (required)</p></td>
    <td><p>Residue number as written in the file.</p></td>
  </tr>
  <tr>
    <td>
      <p><code>insCode</code></p>
    </td>
    <td>
      <p><code>string</code></p>
    </td>
    ...
    <td>
      <p>
        Insertion code, when the residue has one. A blank one and none are the
        same.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        As <code>#RRGGBB</code>. Highlights that name none take the next color
        of <code>HIGHLIGHT_COLOR_PALETTE</code>, in the order they are given.
      </p>
    </td>
  </tr>
</table>
<h3>CameraState</h3>
<p>
  Everything that places the camera, enough to put it back exactly where it was:
  what <code>onCameraChange</code> reports and <code>initialCamera</code> takes.
</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
  ...
  <tr>
    <td>
      <p><code>position</code>, <code>target</code>, <code>up</code></p>
    </td>
    <td>
      <p><code>[number, number, number]</code></p>
    </td>
    <td><p>- (required)</p></td>
    <td>
      <p>Where the camera is, what it looks at, and which way is up.</p>
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td><p>- (required)</p></td>
    <td><p>Field of view, in radians.</p></td>
  </tr>
  <tr>
    <td>
      <p><code>radius</code>, <code>radiusMax</code></p>
    </td>
    <td>
      <p><code>number</code></p>
    </td>
    ...
    <td>
      <p>
        The sphere around <code>target</code> the camera frames, and the whole
        scene's, which bounds how far it clips.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td><p>- (required)</p></td>
    <td><p>The camera's projection.</p></td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      <p><code>{ width: number; height: number }</code></p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Size of the canvas the camera framed, in pixels, for rendering an image
        at the same aspect. Ignored when the state is applied.
      </p>
    </td>
  </tr>
</table>
<h3>StructureLoadInfo</h3>
<p>What <code>onStructureLoad</code> reports for each structure loaded.</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
    <td><p>Type</p></td>
  ...
  <tr>
    <td>
      <p><code>atomCount</code></p>
    </td>
    <td>
      <p><code>number</code></p>
    </td>
    <td><p>- (required)</p></td>
    <td><p>Atoms Mol* parsed.</p></td>
  </tr>
  <tr>
    <td>
      <p><code>residueCount</code></p>
    ...
    <td><p>- (required)</p></td>
    <td>
      <p>Polymer residues across every chain, as the chains count them.</p>
    </td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      <p><code>ChainRef[]</code></p>
    </td>
    <td><p>- (required)</p></td>
    <td><p>The polymer chains found.</p></td>
  </tr>
</table>
<h3>LoadedStructureInfo</h3>
<p>What <code>onReady</code> is handed alongside the plugin.</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
    <td><p>Type</p></td>
  ...
  <tr>
    ...
    <td>
      <p>
        <code
          >StateObjectSelector&lt;PluginStateObject.Molecule.Structure&gt;</code
        >
      </p>
    </td>
    <td><p>- (required)</p></td>
    <td>
      <p>
        The parsed structure's cell in the plugin's state tree, to build
        components and representations on. They go with it when the next
        structure is loaded.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        Atoms Mol* parsed, for checking the parse against a count you hold for
        the file before trusting what is drawn.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        The polymer chains found, as <code>onChainsChange</code> reports them.
      </p>
    </td>
  </tr>
</table>
<h3>StructureSceneOptions</h3>
<p>
  What <code>applyStructureScene</code> draws. <code>structure</code>,
  <code>plddt</code>, <code>residueOverlay</code>, <code>colorBy</code>,
  <code>chainColors</code>, <code>representation</code>,
  <code>highlights</code>, <code>hiddenChains</code>,
  <code>initialCamera</code>, <code>orientation</code> and
  <code>projection</code> mean what the props of the same names mean, with the
  same defaults. Beyond those:
</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
  ...
  <tr>
    ...
    <td>
      <p>
        Which of the neutral grays paints a residue without a value. The viewer
        takes it from the SDS theme.
      </p>
    </td>
  </tr>
  <tr>
    ...
    <td>
      <p>
        <code>(error: unknown, phase: ViewerErrorPhase) =&gt; void</code>
      </p>
    </td>
    <td><p>-</p></td>
    <td>
      <p>
        Told when a representation cannot be drawn, with the phase
        <code>"representation"</code>. Without it the error is logged.
      </p>
    </td>
  </tr>
</table>
<h3>StructureSceneHandle</h3>
<p>What <code>applyStructureScene</code> resolves with.</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
    <td><p>Type</p></td>
  ...
  <tr>
    ...
    <td>
      <p><code>StructureLoadInfo</code></p>
    </td>
    <td><p>What the structure last loaded holds.</p></td>
  </tr>
  <tr>
    <td>
    ...
    <td>
      <p>
        <code
          >(options: Partial&lt;StructureSceneOptions&gt;) =&gt;
          Promise&lt;void&gt;</code
        >
      </p>
    </td>
    <td>
      <p>
        Applies the options given in place, keeping the rest and leaving the
        camera where it is. A new <code>structure</code> is loaded and the
        camera placed on it.
      </p>
    </td>
  </tr>
  <tr>
    <td>
      <p><code>getCamera</code></p>
    </td>
    <td>
      <p><code>() =&gt; CameraState | undefined</code></p>
    </td>
    <td>
      <p>
        Where the camera is now, in the form <code>initialCamera</code> takes.
      </p>
    </td>
  </tr>
  <tr>
...
<h3>RenderStructureImageOptions</h3>
<p>
  What <code>renderStructureImage</code> takes: everything in
  <code>StructureSceneOptions</code>, and the image to draw it into.
</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Field</p></td>
  ...
  <tr>
    ...
    <td>
      <p>Background behind the structure, as <code>#RRGGBB</code>.</p>
    </td>
  </tr>
</table>
`}));export{n,t};