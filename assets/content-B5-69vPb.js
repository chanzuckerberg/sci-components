...
<p>
  The layering of a page rests on three semantic variables:
  <code>background-primary</code>, <code>surface</code>, and
  <code>fill-primary</code>. Where Light Mode uses <code>gray50</code> (white)
  for all of these, Dark Mode has to spread them across two separate shades to
  adhere to visual accessibility best practices.
</p>
<p>
  In Dark Mode, <code>background-primary</code> stays mapped to
  <code>gray50</code>, which inverts to black (<code>#000000</code>), and
  <code>surface</code> and <code>fill-primary</code> are both
  <code>gray75</code> (<code>#1b1b1b</code>, a dark gray that stays visible when
  viewed on top of a <code>gray50</code> background). One of the main reasons
  for this is that drop shadows can't be leveraged in Dark Mode to separate
  layers in the same way that they can be in Light Mode since shadows are not
  nearly as visible when viewed on dark backgrounds.
</p>
<table class="sds-doc-table">
  <tr>
    <td><p>Light Mode</p></td>
