import { z } from "zod";
import {
  fetchComponentPropsStorybook,
  getAllComponentNames,
} from "../lib/fetch.js";
import { type Tool } from "../lib/types.js";

export const getComponentPropsStorybookTool: Tool<{ componentList: string[] }> =
  {
    ...
    exec(server, { ctx, name, description }) {
      server.tool(
        ...
        async ({ component }) => {
          try {
            const json = await fetchComponentPropsStorybook(component);
