...
import {
  getZeroheightConfig,
  type ZeroheightConfig,
} from "./zeroheight-config.js";
...
class ZeroheightExporter {
  ...
  private saveCache(): void {
    try {
      ...
      console.log(
        `✓ Saved cache with ${Object.keys(this.cache.pages).length} pages`
      );
    } catch (error) {
  ...
  private formatSinglePageContent(
    page: ZeroheightPage,
  ...
  private formatCombinedPageContent(pageGroup: PageGroup): string {
    ...
    const sortedPages = pageGroup.pages.sort(
      (a, b) =>
  ...
  private async exportPageGroup(
    ...
    try {
      ...
      const mergePair = mergePairs.find(
        (pair) =>
          pageGroup.baseName === pair.plural &&
          this.pageGroups.has(pair.singular)
      );
      ...
      if (mergePair) {
        ...
        console.log(
          `🔗 Merging ${pageGroup.baseName} content into ${mergePair.singular}.md`
        );
        return {
          success: true,
          fileName: `${mergePair.singular}.md`,
          error: "Merged content",
        };
      }
  ...
  private async generateIndex(
    successfulExports: Array<{ fileName: string }>
  ): Promise<void> {
    const totalPagesCombined = Array.from(this.pageGroups.values()).reduce(
...
${Array.from(this.pageGroups.values())
  .map(
    (group) =>
      `- **${group.baseName}.md**: ${group.pages.length} page${group.pages.length > 1 ? "s" : ""} combined`
  )
  .join("\n")}
`;
  ...
  private async validateComponentAlignment(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "⚠️  Could not validate component alignment:",
        error instanceof Error ? error.message : "Unknown error"
      );
    }
  }
  ...
  async exportAll(): Promise<void> {
    ...
    try {
      ...
      const totalPagesProcessed = Array.from(this.pageGroups.values()).reduce(
        (sum, group) => sum + group.pages.length,
        0
      );
      ...
      if (merged.length > 0) {
        console.log(
          `🔗 Content merged: ${merged.length} plural versions merged into singular files`
        );
      }
      if (failed.length > 0) {
