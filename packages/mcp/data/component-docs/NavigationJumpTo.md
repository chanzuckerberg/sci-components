...
| `onChange`  | `function`               | -            | `(value: number, event?: React.SyntheticEvent, type?: "click" \| "scroll") => void`. Called when the highlighted item changes, with its index and what caused the change. For a scroll the event is synthesized, not a real DOM event. |
