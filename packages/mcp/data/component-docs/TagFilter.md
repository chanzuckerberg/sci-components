...
- The X button is named "Delete Tag" on every filter tag, and the name cannot be changed. The label beside it is read as plain text rather than as part of the button, so a row of filters is worth wrapping in a group with a name of its own — the example below uses _role="group"_ with an _aria-label_ — to say what the buttons are removing.
