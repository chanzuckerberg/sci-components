...
| `dataZoom`            | `EChartsOption["dataZoom"]`                                           | the camera window   | ECharts' dataZoom configuration for that window, merged over the defaults it sets. One object applies to both axes; a two-entry array configures them separately. It reaches the chart only while `camera.active` is `true`. |
