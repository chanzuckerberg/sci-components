...
| `onSelectionChange` | `function`         | -            | `(selectedIndices: number[]) => void`. Called with the next selection when an item is clicked, with that index toggled. Without it, clicking selects nothing.                                                     |
...
| `onItemClick`       | `function`         | -            | `(item: LegendItemData, index: number) => void`. Fires on a click, alongside any selection change rather than instead of it.                                                                                      |
| `onItemMouseEnter`  | `function`         | -            | `(item: LegendItemData, index: number) => void`. Called when the pointer enters an item. The legend already dims its own swatches, so this is for driving something outside it, such as a highlight on the chart. |
| `onItemMouseLeave`  | `function`         | -            | `(item: LegendItemData, index: number) => void`. Called when the pointer leaves an item.                                                                                                                          |
