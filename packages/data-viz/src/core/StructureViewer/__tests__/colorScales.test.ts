...
describe("sampleSteppedScale", () => {
  it("puts each threshold at the bottom of the band above it", () => {
    ...
    expect(sampleSteppedScale(PLDDT_STOPS, 0.69)).toEqual(hexToRgb("#FFDB11"));
    expect(sampleSteppedScale(PLDDT_STOPS, 0.7)).toEqual(hexToRgb("#64CBF3"));
  });

  it("returns the lowest band below the first threshold", () => {
    expect(sampleSteppedScale(PLDDT_STOPS, 0)).toEqual(hexToRgb("#FF7C45"));
  });

  it("returns the highest band at the top of the range", () => {
    expect(sampleSteppedScale(PLDDT_STOPS, 1)).toEqual(hexToRgb("#0053D5"));
  });
});
...
describe("sampleColorScale", () => {
  ...
  it("samples stepped scales through the same entry point", () => {
    expect(sampleColorScale(PLDDT_COLOR_SCALE, 0.95, 1)).toEqual(
      hexToRgb("#0053D5")
    );
  });
});
